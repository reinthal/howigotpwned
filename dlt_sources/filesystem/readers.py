...
def _read_zips(
    ...
    """A resource to extract files from zip and return each file with their full path.
    

    Args:
        items (Iterator[FileItemDict]): zip files to read.

    Returns:
        Iterable[TDataItem]: Data items, read from the given zip files.
    """
    import zipfile
    for item in items:
...
if TYPE_CHECKING:
    ...
    class ReadersSource(DltSource):
        ...
        @copy_sig(_read_zips)
        def read_zips(self) -> DltResource:
            ...
